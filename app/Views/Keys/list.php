        ...
        <div class="col-lg-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                        <div class="col pt-1">Keys List</div>
                        <div class="col text-end">
                           <a class="btn btn-outline-light btn-sm" href="<?= site_url('keys/generate') ?>"><i class="bi bi-person-plus "></i> Gen Key</a>
                            <a class="btn btn-outline-dark btn-sm ms-1" id="blur-out" data-bs-toggle="tooltip" data-bs-placement="top" title="Eye Protect"><i class="bi bi-eye-slash"></i></a>
                            <?php if($user->level == 1) : ?>
                            <a class="btn btn-outline-danger btn-sm" href="<?= site_url('keys/delete') ?>"><i class="bi bi-trash-fill"></i> All</a><br><br>
                            <?php endif; ?>
                            <a class="btn btn-outline-light btn-sm" href="<?= site_url('keys/start')  ?>"><i class="bi bi-trash-fill"></i> Un-Used</a>
                            <?php if($user->level == 1) : ?>
                            <a class="btn btn-outline-warning btn-sm" href="<?= site_url('keys/alter') ?>"><i class="bi bi-trash-fill"></i> Used</a>
                            <?php endif; ?>
                            <a class="btn btn-outline-light btn-sm" href="<?= site_url('keys/download/all') ?>"><i class="bi bi-download"></i> All</a>
                    </div>
                 </div>
              </div>
                        ...
                        if (textStatus == 'success') {
                            if (data.registered) {
                                if (data.reset) {
                                    ...
                                    Swal.fire(
                                        'Reset',
                                        'The key has been successfully reseted',
                                        'success'
                                    )
                                } else {
                                    Swal.fire(
                                        'Failed',
                                        data.devices_total ? "You don't have access to this key" : "User key devices already reset.",
