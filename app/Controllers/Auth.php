...
class Auth extends BaseController
{
    ...
    public function login()
    {
        if (session()->has('userid'))
            return redirect()->to('dashboard')->with('msgSuccess', 'Successfully logged in');
        ...
        if (!$this->validate($form_rules)) {
            ...
            if ($cekUser) {
                ...
                if (password_verify($hashPassword, $cekUser->password)) {
                    ...
                    return redirect()->route('login')->withInput()->with('msgDanger', '<strong>Expired Account</strong> Please Renew Your Account to Login');
    ...
    public function register_action()
    {
        ...
        if (!$this->validate($form_rules)) {
            ...
            $validation = Services::validation();
            if (!$rCheck) {
                $validation->setError('referral', 'Wrong referral');
            } else {
                if ($rCheck->used_by) {
                    $validation->setError('referral', "Wrong referral code, referral code has been used &middot; $rCheck->used_by.");
                } else {
    ...
    public function logout()
    {
        if (session()->has('userid')) {
            ...
            session()->setFlashdata('msgSuccess', 'Successfuly logged out');
        }
