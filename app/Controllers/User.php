...
//=================================================//
    ...
    private function passwd_act()
    {
        ...
        if($row)
        {
            ...
            $link = "https://".$server."/verify_pass?key=".$newPassword."&mail=".$emailId."&token=".$token.'';
            $email = \Config\Services::email();
            $email->setFrom('panel@just-panel.xyz', 'Panel By Spencer');
            $email->setTo($emailId);
            $email->setSubject("[$server] Password Change Request");
            $email->setMessage("Hello!
You have been requested to change your password at $server
for $username] at $timestamp
Please, Click on the following link to change your Accounts Password
